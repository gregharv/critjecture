...
import type { PredictiveRunDetail } from "@/lib/predictive-analysis";
...
type PredictiveRunPageClientProps = {
  initialRunDetail: PredictiveRunDetail;
};
...
export function PredictiveRunPageClient({ initialRunDetail }: PredictiveRunPageClientProps) {
  ...
  async function refreshRun() {
    ...
    try {
      const response = await fetch(`/api/predictive/runs/${runDetail.run.id}`, {
        cache: "no-store",
      });
      const json = (await response.json()) as unknown;

      if (!response.ok) {
        throw new Error(getErrorMessage(json, "Failed to refresh predictive run."));
      }

      setRunDetail(json as PredictiveRunDetail);
    } catch (caughtError) {
      setError(caughtError instanceof Error ? caughtError.message : "Failed to refresh predictive run.");
    } finally {
      setPending(false);
    }
  }
  ...
  async function handleGenerateAnswer() {
    ...
    try {
      const response = await fetch(`/api/predictive/runs/${runDetail.run.id}/answers`, {
        method: "POST",
      });
      const json = (await response.json()) as unknown;

      if (!response.ok) {
        throw new Error(getErrorMessage(json, "Failed to generate grounded predictive answer."));
      }

      await refreshRun();
    } catch (caughtError) {
      setError(
        caughtError instanceof Error ? caughtError.message : "Failed to generate grounded predictive answer.",
      );
    } finally {
      setPending(false);
    }
  ...
  return (
    <section className="causal-page">
      <div className="causal-hero">
        <p className="causal-hero__eyebrow">Predictive run detail</p>
        <h1 className="causal-hero__title">{runDetail.dataset?.displayName ?? "Predictive run"}</h1>
        ...
        <p className="causal-card__meta" style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <Link className="causal-study-list__link" href="/predictive">
            ← Back to predictive workspace
          </Link>
          <a
            className="causal-study-list__link"
            href={`/api/predictive/runs/${runDetail.run.id}/export`}
          >
      ...
      <div className="causal-grid">
        <section className="causal-card">
          <div className="causal-card__header-row">
            <h2 className="causal-card__title">Grounded predictive answer</h2>
            <button
          ...
          {latestAnswer ? (
            <>
              ...
              <div className="causal-grid">
                <section className="causal-card">
                  <h3 className="causal-card__title">Metrics</h3>
                  {metricEntries.length ? (
                    <ul className="causal-study-list">
                      {metricEntries.map(([metric, value]) => (
                        <li key={metric} className="causal-study-list__item">
                          <div className="causal-study-list__header">
                            <strong>{formatLabel(metric)}</strong>
                            <span className="causal-study-list__status">{formatNumber(value)}</span>
                          </div>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="causal-card__empty">No predictive metrics were stored in the grounded package.</p>
                  )}
                </section>

                <section className="causal-card">
                  <h3 className="causal-card__title">Top feature signals</h3>
                  {featureImportanceEntries.length ? (
                    <ul className="causal-study-list">
                      {featureImportanceEntries.slice(0, 5).map(([feature, value]) => (
                        <li key={feature} className="causal-study-list__item">
                          <div className="causal-study-list__header">
                    ...
                    <p className="causal-card__empty">No feature importance values were stored in the grounded package.</p>
                  )}
                </section>
              </div>

              <details>
                <summary className="causal-card__meta">Raw grounded answer markdown</summary>
        ...
        <section className="causal-card">
          <div className="causal-card__header-row">
            ...
            <button className="causal-intake-form__submit" disabled={pending} onClick={() => void refreshRun()} type="button">
          ...
          <h3 className="causal-card__title">Package limitations</h3>
          {parsedPackage?.limitations.length ? (
            ...
            <p className="causal-card__empty">No package limitations were stored.</p>
          )}

          <h3 className="causal-card__title">Suggested next steps</h3>
      ...
      <div className="causal-grid">
        <section className="causal-card">
          <h2 className="causal-card__title">Stored predictive result</h2>
          {runDetail.result ? (
            <>
              <p className="causal-card__copy">
              ...
              <p className="causal-card__copy">{runDetail.result.summaryText}</p>

              <h3 className="causal-card__title">All metrics</h3>
              {Object.entries(runDetail.result.metrics).length ? (
                <ul className="causal-study-list">
                  {Object.entries(runDetail.result.metrics).map(([key, value]) => (
                    <li key={key} className="causal-study-list__item">
                      <div className="causal-study-list__header">
                ...
                <p className="causal-card__empty">No predictive metrics were stored for this run.</p>
              )}

              <h3 className="causal-card__title">All feature importance</h3>
              {featureImportanceEntries.length ? (
                <ul className="causal-study-list">
                ...
                <p className="causal-card__empty">No feature importance values were stored for this run.</p>
              )}

              <details>
                <summary className="causal-card__meta">Raw result JSON</summary>
                <div className="causal-answer-markdown">
            ...
            <p className="causal-card__empty">
              No persisted predictive result is available for this run yet.
            </p>
          )}
        </section>
      </div>
      ...
      <div className="causal-grid">
        <section className="causal-card">
          <h2 className="causal-card__title">Execution telemetry</h2>
          {runDetail.computeRuns.length ? (
              ...
              {runDetail.computeRuns.map((computeRun) => (
                <li key={computeRun.id} className="causal-study-list__item">
                  ...
                  {computeRun.failureReason ? (
                    <p className="causal-intake-form__error">{computeRun.failureReason}</p>
                  ) : null}
                  <details>
                    <summary className="causal-card__meta">Telemetry payloads</summary>
                    <div className="causal-answer-markdown">
            ...
            <p className="causal-card__empty">No compute telemetry was recorded for this run.</p>
          )}
        </section>

        <section className="causal-card">
          <h2 className="causal-card__title">Artifacts</h2>
          {runDetail.artifacts.length ? (
            ...
            <p className="causal-card__empty">No downloadable artifacts were recorded for this run.</p>
          )}
        </section>
      </div>
    </section>
