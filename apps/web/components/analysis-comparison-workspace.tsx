...
import { AnalysisComparisonUtilities } from "@/components/analysis-comparison-utilities";
...
type AnalysisComparisonWorkspaceProps = {
  bestSupportedIdentifiedRun: AnalysisRunSummary | null;
  comparisonAdjustmentDiff: { added: string[]; removed: string[] };
  comparisonBaseRun: AnalysisRunSummary | null;
  ...
  comparisonTargetRun: AnalysisRunSummary | null;
  ...
  latestAnswerBearingRun: AnalysisRunSummary | null;
  latestCompletedRun: AnalysisRunSummary | null;
...
export function AnalysisComparisonWorkspace({
  ...
  runs,
  studyId,
}: AnalysisComparisonWorkspaceProps) {
  if (runs.length < 2) {
    return null;
  }
  ...
  return (
    <div aria-busy={comparisonPending} style={COMPARISON_PANEL_STYLE}>
      <div className="analysis-card__header-row">
        <h3 className="analysis-card__title" style={{ fontSize: 16 }}>Run comparison</h3>
        <div className="analysis-inline-actions">
          <span className="analysis-card__meta">Compare identification and estimate changes across runs. The current pair stays encoded in the URL.</span>
          {comparisonBaseRunId && comparisonTargetRunId ? (
            <>
              <button className="analysis-inline-button" onClick={onCopyComparisonLink} type="button">
              ...
              <a
                className="analysis-study-list__link"
                href={`/api/analysis/studies/${studyId}/compare-export?baseRunId=${encodeURIComponent(comparisonBaseRunId)}&targetRunId=${encodeURIComponent(comparisonTargetRunId)}`}
              >
      ...
      {comparisonBaseRun && comparisonTargetRun ? (
        <div className="analysis-grid" style={{ marginTop: 0 }}>
          {[comparisonBaseRun, comparisonTargetRun].map((run, index) => (
            <section key={run.id} className="analysis-card">
              <div className="analysis-card__header-row">
                <strong>{index === 0 ? "Baseline" : "Comparison"}</strong>
                <span className="analysis-study-list__status">{formatLabel(run.status)}</span>
              </div>
              <p className="analysis-card__meta">
                <Link className="analysis-study-list__link" href={`/analysis/studies/${studyId}/runs/${run.id}`}>
                  {run.id}
                </Link>
              </p>
              ...
              <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
                <div>
                  <strong className="analysis-card__meta">Adjustment set</strong>
                  {run.adjustmentSet.length ? (
                    ...
                    <p className="analysis-card__meta">No stored adjustment set.</p>
                  )}
                </div>
                <div>
                  <strong className="analysis-card__meta">Estimands</strong>
                  {run.estimandLabels.length ? (
                    ...
                    <p className="analysis-card__meta">No stored estimands.</p>
                  )}
                </div>
                <div>
                  <strong className="analysis-card__meta">Blocking reasons</strong>
                  {run.blockingReasons.length ? (
                    ...
                    <p className="analysis-card__meta">No stored blocking reasons.</p>
                  )}
                </div>
                <div>
                  <strong className="analysis-card__meta">Refuters</strong>
                  {run.refuterNames.length ? (
                    ...
                    <p className="analysis-card__meta">No stored refuters.</p>
                  )}
                </div>
              </div>
            </section>
          ...
          <section className="analysis-card">
            <h3 className="analysis-card__title">Delta summary</h3>
            <div style={{ display: "grid", gap: 12 }}>
              <div>
                <strong className="analysis-card__meta">Headline changes</strong>
                <ul className="analysis-list">
              ...
              <div>
                <strong className="analysis-card__meta">Support deltas</strong>
                <ul className="analysis-list">
              ...
              <div>
                <strong className="analysis-card__meta">Adjustment set diff</strong>
                <ul className="analysis-list">
              ...
              <div>
                <strong className="analysis-card__meta">Estimand diff</strong>
                <ul className="analysis-list">
              ...
              <div>
                <strong className="analysis-card__meta">Blocking reason diff</strong>
                <ul className="analysis-list">
              ...
              <div>
                <strong className="analysis-card__meta">Refuter diff</strong>
                <ul className="analysis-list">
