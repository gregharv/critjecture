...
type AnalysisRunPageClientProps = {
...
export function AnalysisRunPageClient({
  initialRunDetail,
  study,
}: AnalysisRunPageClientProps) {
  ...
  const derivedEpistemicVerdict = deriveAnalysisEpistemicVerdict({
  ...
  async function refreshRun() {
    ...
    try {
      const response = await fetch(`/api/analysis/runs/${runDetail.run.id}`, {
        cache: "no-store",
      });
      const json = (await response.json()) as unknown;

      if (!response.ok) {
        throw new Error(getErrorMessage(json, "Failed to refresh analysis run."));
      }

      setRunDetail(json as RunDetail);
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : "Failed to refresh analysis run.");
    } finally {
      setPending(false);
    }
  }
  ...
  async function handleGenerateAnswer() {
    ...
    try {
      const response = await fetch(`/api/analysis/runs/${runDetail.run.id}/answers`, {
        method: "POST",
      });
      const json = (await response.json()) as unknown;
  ...
  return (
    <section className="analysis-page">
      <div className="analysis-hero">
        <p className="analysis-hero__eyebrow">Analysis run detail</p>
        <h1 className="analysis-hero__title">{study.title}</h1>
        ...
        <p className="analysis-card__meta" style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
          <Link className="analysis-study-list__link" href={`/analysis/studies/${study.id}`}>
            ← Back to study workspace
          </Link>
          <a className="analysis-study-list__link" href={`/api/analysis/runs/${runDetail.run.id}/export`}>
      ...
      <div className="analysis-grid">
        {[
          ...
          <section key={card.label} className="analysis-card">
            <p className="analysis-card__meta">{card.label}</p>
            <div
              style={{
                ...card.tone,
                borderRadius: 14,
      ...
      <div className="analysis-grid">
        <section className="analysis-card">
          <div className="analysis-card__header-row">
            <h2 className="analysis-card__title">Grounded final answer</h2>
          ...
          <p className="analysis-card__copy">
            Final answers are rendered from the stored analysis answer package only. This page is for
            interpreting stored higher-rung evidence, assumptions, and refutation outputs.
          </p>
          ...
          {latestAnswer ? (
            <>
              ...
              <div className="analysis-grid" style={{ marginTop: 16 }}>
                <section className="analysis-card">
                  <p className="analysis-card__meta">Topline claim</p>
                  <div
                    style={{
                  ...
                  <p className="analysis-card__copy" style={{ marginTop: 12 }}>
                    {epistemicVerdict.summaryText}
                  </p>
                  <p className="analysis-card__meta" style={{ marginTop: 12 }}>
                    {identified === false
                ...
                <section className="analysis-card">
                  <p className="analysis-card__meta">Evidence snapshot</p>
                  <ul className="analysis-list">
                ...
                <section className="analysis-card">
                  <p className="analysis-card__meta">Key assumptions</p>
                  {assumptionHighlights.length ? (
                    ...
                    <p className="analysis-card__empty">No explicit assumptions were stored.</p>
                  )}
                </section>

                <section className="analysis-card">
                  <p className="analysis-card__meta">Main caveats</p>
                  {identificationBlockingReasons.length || limitationHighlights.length ? (
                    ...
                    <p className="analysis-card__empty">No blocking reasons or package limitations were stored.</p>
                  )}
                </section>
              </div>

              <details open>
                <summary className="analysis-card__meta">Latest grounded answer markdown</summary>
                <div className="analysis-answer-markdown">
          ...
          {runDetail.answers.length ? (
            <>
              <h3 className="analysis-card__title">Answer history</h3>
              <ul className="analysis-study-list">
                {runDetail.answers.map((answer, index) => (
                  <li key={answer.id} className="analysis-study-list__item">
                    ...
                    <details>
                      <summary className="analysis-card__meta">View stored markdown</summary>
                      <div className="analysis-answer-markdown">
        ...
        <section className="analysis-card">
          <h2 className="analysis-card__title">Run grounding and status</h2>
          <ul className="analysis-list">
          ...
          {parsedPackage?.approval ? (
            ...
            >
              ...
              {parsedPackage.approval.approvalText ? (
                <p className="analysis-card__copy">{parsedPackage.approval.approvalText}</p>
              ) : null}
            </div>
          ) : null}
          <details>
            <summary className="analysis-card__meta">Raw answer package JSON</summary>
            <div className="analysis-answer-markdown">
      ...
      <div className="analysis-grid">
        <section className="analysis-card">
          <h2 className="analysis-card__title">Identification and estimates</h2>
          <p className="analysis-card__copy">
          ...
          {identificationBlockingReasons.length ? (
            ...
            >
              <h3 className="analysis-card__title" style={{ color: "#b91c1c", fontSize: 16 }}>
                Blocking reasons
              </h3>
              <ul className="analysis-list">
          ...
          {runDetail.estimates.length ? (
            <ul className="analysis-study-list">
              {runDetail.estimates.map((estimate, index) => (
                <li key={estimate.id} className="analysis-study-list__item">
                  <div className="analysis-study-list__header">
                    <strong>{index === 0 ? `${estimate.effectName} (primary)` : estimate.effectName}</strong>
                    <span className="analysis-study-list__status">{estimate.estimatorName}</span>
                  </div>
                  <p className="analysis-study-list__meta">
            ...
            <p className="analysis-card__empty">No estimate was stored for this run.</p>
          )}

          {runDetail.estimands.length ? (
            <>
              <h3 className="analysis-card__title">Identified estimands</h3>
              <ul className="analysis-study-list">
                {runDetail.estimands.map((estimand) => (
                  <li key={estimand.id} className="analysis-study-list__item">
                    <div className="analysis-study-list__header">
                      <strong>{estimand.estimandLabel}</strong>
                      <span className="analysis-study-list__status">{estimand.estimandKind}</span>
                    </div>
                    <details>
                      <summary className="analysis-card__meta">View estimand expression</summary>
                      <div className="analysis-answer-markdown">
          ...
          {identificationAdjustmentSet.length ? (
            <>
              <h3 className="analysis-card__title">Adjustment set</h3>
              <ul className="analysis-list">
          ...
          <details>
            <summary className="analysis-card__meta">Raw identification record</summary>
            <div className="analysis-answer-markdown">
        ...
        <section className="analysis-card">
          <h2 className="analysis-card__title">Assumptions, limitations, and next steps</h2>
          ...
          {parsedPackage?.assumptions.length ? (
            <ul className="analysis-study-list">
              {parsedPackage.assumptions.map((assumption, index) => (
                <li key={`${assumption.description ?? assumption.assumptionType ?? "assumption"}-${index}`} className="analysis-study-list__item">
                  <div className="analysis-study-list__header">
                    <strong>{assumption.description ?? assumption.assumptionType ?? "Unnamed assumption"}</strong>
                    <span className="analysis-study-list__status">{assumption.status ?? "not labeled"}</span>
                  </div>
                  {assumption.assumptionType ? (
                    <p className="analysis-study-list__meta">Type {assumption.assumptionType}</p>
                  ) : null}
                </li>
              ))}
            </ul>
          ...
          <h3 className="analysis-card__title">Limitations</h3>
          {parsedPackage?.limitations.length ? (
            ...
            <p className="analysis-card__empty">No additional limitations were stored.</p>
          )}

          <h3 className="analysis-card__title">Next steps</h3>
          {parsedPackage?.nextSteps.length ? (
            ...
            <p className="analysis-card__empty">No next steps were stored.</p>
          )}
        </section>
      </div>
      ...
      <div className="analysis-grid">
        <section className="analysis-card">
          <h2 className="analysis-card__title">Refutations</h2>
          {runDetail.refutations.length ? (
            <ul className="analysis-study-list">
              {runDetail.refutations.map((refutation) => (
                <li key={refutation.id} className="analysis-study-list__item">
                  <div className="analysis-study-list__header">
                    <strong>{refutation.refuterName}</strong>
                    <span className="analysis-study-list__status">{refutation.status}</span>
                  </div>
                  <p className="analysis-study-list__meta">{refutation.summaryText}</p>
                  <p className="analysis-study-list__meta">Recorded {formatTimestamp(refutation.createdAt)}</p>
                </li>
              ))}
            </ul>
            ...
            <ul className="analysis-study-list">
              {parsedPackage.refutations.map((refutation, index) => (
                <li key={`${refutation.refuterName ?? "refutation"}-${index}`} className="analysis-study-list__item">
                  <div className="analysis-study-list__header">
                    <strong>{refutation.refuterName ?? "Unnamed refuter"}</strong>
                    <span className="analysis-study-list__status">{refutation.status ?? "not labeled"}</span>
                  </div>
                  <p className="analysis-study-list__meta">{refutation.summaryText ?? "No summary stored."}</p>
                </li>
              ))}
            </ul>
          ) : (
            <p className="analysis-card__empty">No refutation rows were stored for this run.</p>
          )}
        </section>

        <section className="analysis-card">
          <h2 className="analysis-card__title">Compute history and artifacts</h2>
          {latestComputeRun ? (
            <p className="analysis-card__copy">
          ...
          {runDetail.computeRuns.length ? (
            <ul className="analysis-study-list">
              {runDetail.computeRuns.map((computeRun) => (
                <li key={computeRun.id} className="analysis-study-list__item">
                  <div className="analysis-study-list__header">
                    <strong>{computeRun.computeKind}</strong>
                    <span className="analysis-study-list__status">{computeRun.status}</span>
                  </div>
                  <p className="analysis-study-list__meta">
            ...
            <p className="analysis-card__empty">No compute runs recorded.</p>
          )}

          {runDetail.artifacts.length ? (
            <>
              <h3 className="analysis-card__title">Artifacts</h3>
              <ul className="analysis-study-list">
                {runDetail.artifacts.map((artifact) => (
                  <li key={artifact.id} className="analysis-study-list__item">
                    <div className="analysis-study-list__header">
                      <strong>{artifact.fileName}</strong>
                      <span className="analysis-study-list__status">{artifact.artifactKind}</span>
                    </div>
                    <p className="analysis-study-list__meta">
                      Stored {formatTimestamp(artifact.createdAt)} · {artifact.mimeType}
                    </p>
                    <p className="analysis-card__meta">
                      <a className="analysis-study-list__link" href={artifact.downloadPath}>
                        Download artifact
                      </a>
                    </p>
                    <details>
                      <summary className="analysis-card__meta">View storage path</summary>
                      <div className="analysis-answer-markdown">
            ...
            <p className="analysis-card__empty">No artifacts were stored for this run.</p>
          )}
        </section>
      </div>
    </section>
