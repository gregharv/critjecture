...
import { AnalysisExportError, exportAnalysisRunZip } from "@/lib/analysis-export";
...
export async function GET(
  ...
  try {
    ...
    if (caughtError instanceof AnalysisExportError) {
      const statusCode = caughtError.code === "analysis_run_not_found" ? 404 : 400;
      return jsonError(caughtError.message, statusCode);
    }

    return jsonError(
      caughtError instanceof Error ? caughtError.message : "Failed to export analysis run.",
      500,
    );
  }
}
