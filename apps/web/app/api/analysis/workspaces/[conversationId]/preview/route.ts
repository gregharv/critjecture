...
export async function GET(
  ...
  const observed = beginObservedRequest({
    method: "GET",
    routeGroup: "sandbox",
    routeKey: "analysis.workspace.preview",
    user,
  });
  ...
  try {
    ...
    return finalizeObservedRequest(observed, {
      ...
      usageEvents: [
        {
          eventType: "analysis_preview_bootstrap",
          metadata: {
            previewSessionId: preview.sessionId,
            revisionId: preview.revisionId,
            workspaceId: preview.workspaceId,
    ...
    const message =
      caughtError instanceof Error ? caughtError.message : "Failed to start analysis preview.";
    const status = /not found/i.test(message) ? 404 : 500;

    return finalizeObservedRequest(observed, {
      errorCode: "analysis_preview_failed",
      metadata: {
        conversationId: normalizedConversationId,
      },
      outcome: "error",
