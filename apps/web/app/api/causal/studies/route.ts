...
import { createCausalStudy, createStudyQuestion, listCausalStudiesForOrganization } from "@/lib/causal-studies";
...
export async function POST(request: Request) {
  ...
  if (!questionText) {
    return jsonError("questionText is required when creating a causal study manually.", 400);
  }

  await ensureCausalFoundationForUser(user);
  const study = await createCausalStudy({
