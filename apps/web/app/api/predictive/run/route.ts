...
function parseRequestBody(body: PredictiveRunRequest):
  | { error: string }
  | {
...
export async function POST(request: Request) {
  ...
  if (!user.access.canUseAnswerTools) {
    return jsonError("This membership cannot run predictive analysis tools.", 403);
  }

  let body: PredictiveRunRequest;

  try {
    body = (await request.json()) as PredictiveRunRequest;
  } catch {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Predictive analysis failed.";
