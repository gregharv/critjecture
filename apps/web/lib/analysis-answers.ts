...
import { analysisAnswerPackages, analysisAnswers, analysisRuns, analysisStudies } from "@/lib/app-schema";

export const ANALYSIS_ANSWER_MODEL_NAME = "grounded-analysis-package-template";
export const ANALYSIS_ANSWER_PROMPT_VERSION = "analysis_answer_markdown_v1";

type ParsedAnalysisAnswerPackage = {
  assumptions?: Array<{
...
function parsePackageJson(packageJson: string) {
  const parsed = JSON.parse(packageJson) as ParsedAnalysisAnswerPackage;
...
function renderEstimateSummary(
  ...
  if (!estimate || typeof estimate.estimateValue !== "number") {
    return "No identified estimate is available in the stored package.";
  }
  ...
  return `The stored estimate for **${estimate.effectName ?? "the treatment effect"}** is **${formatNumber(estimate.estimateValue)}** using **${estimate.estimatorName ?? "the recorded estimator"}**${intervalText}.`;
}
...
function renderGroundedAnswerMarkdown(packageJson: string) {
  ...
  const derivedVerdict = parsed.epistemicVerdict ?? deriveAnalysisEpistemicVerdict({
  ...
  if (parsed.nextSteps.length) {
    ...
    lines.push("- Resolve the blocking assumptions or missing variables before interpreting any higher-rung effect estimate.");
  }

  lines.push(
    "",
    "## Guardrail note",
    "This answer was rendered from the stored analysis answer package only. It did not re-analyze the dataset or use descriptive analysis tools.",
  );

  return lines.join("\n");
}

export async function listAnalysisAnswersForStudy(input: {
  ...
  return db
    ...
    .where(and(eq(analysisAnswers.organizationId, input.organizationId), eq(analysisAnswers.studyId, input.studyId)))
    .orderBy(desc(analysisAnswers.createdAt));
}

export async function listAnalysisAnswersForRun(input: {
  organizationId: string;
  runId: string;
}) {
  const db = await getAppDatabase();
  return db
    .select()
    .from(analysisAnswers)
    .where(and(eq(analysisAnswers.organizationId, input.organizationId), eq(analysisAnswers.runId, input.runId)))
    .orderBy(desc(analysisAnswers.createdAt));
}

export async function getLatestAnalysisAnswerForRun(input: {
  organizationId: string;
  runId: string;
}) {
  const answers = await listAnalysisAnswersForRun(input);
  return answers[0] ?? null;
}

export async function createGroundedAnalysisAnswer(input: {
  organizationId: string;
  runId: string;
}) {
  const db = await getAppDatabase();
  ...
  const [answerPackage] = await db
    ...
    .where(
      and(
        eq(analysisAnswerPackages.runId, run.id),
        eq(analysisAnswerPackages.studyId, run.studyId),
        eq(analysisAnswerPackages.organizationId, input.organizationId),
      ),
    );

  if (!answerPackage) {
    throw new Error("A analysis answer package must exist before generating the final answer.");
  }
  ...
  await db.insert(analysisAnswers).values({
    id,
    runId: run.id,
    studyId: run.studyId,
    organizationId: input.organizationId,
    answerPackageId: answerPackage.id,
    modelName: ANALYSIS_ANSWER_MODEL_NAME,
    promptVersion: ANALYSIS_ANSWER_PROMPT_VERSION,
    answerText,
  ...
  await db
    ...
    .where(eq(analysisStudies.id, run.studyId));

  const [storedAnswer] = await db.select().from(analysisAnswers).where(eq(analysisAnswers.id, id));
  return storedAnswer ?? null;
}

export function parseAnalysisAnswerPackageForDisplay(packageJson: string) {
