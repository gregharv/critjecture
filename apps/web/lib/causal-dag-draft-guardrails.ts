...
export function evaluateDraftDagGuardrails(input: {
  ...
  for (const assumption of input.draft.assumptions) {
    ...
    if (
      assumption.status &&
      !(CAUSAL_ASSUMPTION_STATUS_VALUES as readonly string[]).includes(assumption.status)
    ) {
  ...
  for (const requirement of input.draft.dataRequirements) {
    if (
      !(CAUSAL_DATA_REQUIREMENT_STATUS_VALUES as readonly string[]).includes(requirement.status ?? "missing")
    ) {
