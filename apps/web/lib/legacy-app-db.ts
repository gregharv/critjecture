...
import { resolveLegacyDatabaseFilePath, resolveWebRoot } from "@/lib/app-paths";
import * as schema from "@/lib/legacy-app-schema";

const MIGRATIONS_TABLE = "__critjecture_migrations";

type LegacyAppDatabase = ReturnType<typeof drizzle<typeof schema>>;

let databaseInstance: LegacyAppDatabase | null = null;
...
async function resolveMigrationPaths() {
  ...
  return {
    dbFilePath: await resolveLegacyDatabaseFilePath(),
...
export async function ensureLegacyDatabaseReady() {
  if (databaseInstance) {
    if (migrationPromise) {
      await migrationPromise;
    }
...
export const getLegacyAppDatabase = ensureLegacyDatabaseReady;
export const getAppDatabase = ensureLegacyDatabaseReady;

export async function resetLegacyAppDatabaseForTests() {
...
export const resetAppDatabaseForTests = resetLegacyAppDatabaseForTests;
