...
import { loadCausalDagExecutionGraph, deriveIdentificationPlan } from "@/lib/causal-graph";
import { buildAndStoreCausalAnswerPackage, getCausalRunPackage } from "@/lib/causal-result-package";
...
type ResolvedCausalRunner = {
  ...
  pythonPath: string;
  requestedKind: typeof causalRuns.$inferInsert.runnerKind;
  runnerKind: typeof causalRuns.$inferInsert.runnerKind;
...
async function resolveCausalRunner(policy: Awaited<ReturnType<typeof getOrganizationExecutionPolicy>>) {
  ...
  if (requestedKind !== "hybrid" && dedicatedPythonPath) {
    ...
    return {
      ...
      fallbackReason: "Dedicated causal runner does not currently expose DoWhy; falling back to hybrid execution.",
  ...
  return {
    capabilities: await getRunnerCapabilities(localPythonPath),
    fallbackReason: requestedKind === "hybrid" ? null : "No dedicated causal runner configured; falling back to hybrid execution.",
    pythonPath: localPythonPath,
    requestedKind,
    runnerKind: "hybrid",
    runnerVersion: await getRunnerVersion(localPythonPath),
...
async function getLatestApprovalForDagVersion(dagVersionId: string) {
  ...
  const rows = await db
    ...
    .where(eq(causalApprovals.dagVersionId, dagVersionId))
...
async function createQueuedRun(input: {
  ...
  await db.insert(causalRuns).values({
    id: runId,
    studyId: input.studyId,
    organizationId: input.organizationId,
...
async function updateRunStatus(input: {
  ...
  status: typeof causalRuns.$inferInsert.status;
}) {
  const db = await getAppDatabase();
  await db
    .update(causalRuns)
    ...
    .where(eq(causalRuns.id, input.runId));
}
...
async function pinRunDatasetBindings(input: {
  ...
  for (const binding of bindings) {
    ...
    await db.insert(causalRunDatasetBindings).values({
      id: randomUUID(),
      runId: input.runId,
      organizationId: input.organizationId,
...
async function getStudyExecutionContext(input: { organizationId: string; studyId: string }) {
  ...
  if (!study.currentQuestionId) {
    throw new Error("A causal study question is required before run creation.");
  }
...
async function persistEstimateRow(input: {
  ...
  await db.insert(causalEstimates).values({
    id: randomUUID(),
    runId: input.runId,
...
async function persistRefutationRow(input: {
  ...
  await db.insert(causalRefutations).values({
    id: randomUUID(),
    runId: input.runId,
    organizationId: input.organizationId,
...
export async function createAndExecuteCausalRun(input: {
  runUser: RunUserContext;
  studyId: string;
}) {
  ...
  const graph = await loadCausalDagExecutionGraph({
  ...
  if ((policy?.requireDagApproval ?? true) && !latestApproval) {
    throw new Error("An approval for the exact DAG version is required before creating a causal run.");
  }
  ...
  try {
    ...
    await db.insert(causalIdentifications).values({
      id: randomUUID(),
      runId,
      organizationId: input.runUser.organizationId,
      identified: identificationPlan.identifiable,
    ...
    if (!identificationPlan.identifiable) {
      ...
      await buildAndStoreCausalAnswerPackage({
        organizationId: input.runUser.organizationId,
        runId,
        studyId: study.id,
      });
      const storedPackage = await getCausalRunPackage(runId);
      if (storedPackage) {
        await ensureRunArtifact({
      ...
      return getCausalRunDetail({ organizationId: input.runUser.organizationId, runId });
    }
    ...
    const message = error instanceof Error ? error.message : "Causal run execution failed.";
    await failComputeRun({
      computeRunId: identificationComputeRunId,
...
export async function listCausalRunsForStudy(input: {
  organizationId: string;
  studyId: string;
}) {
  const db = await getAppDatabase();
