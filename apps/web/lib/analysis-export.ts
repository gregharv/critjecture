...
export class AnalysisExportError extends Error {
  ...
  constructor(message: string, code = "analysis_export_error") {
    super(message);
    this.code = code;
...
function sanitizeArchiveSegment(value: string) {
  ...
  return normalized || "analysis-run";
}
...
function buildArchiveFileName(input: {
  ...
  const studySegment = sanitizeArchiveSegment(input.studyTitle ?? "analysis-run");
...
function buildComparisonArchiveFileName(input: {
  ...
  const studySegment = sanitizeArchiveSegment(input.studyTitle ?? "analysis-comparison");
  return `${studySegment}-compare-${sanitizeArchiveSegment(input.baseRunId)}-vs-${sanitizeArchiveSegment(input.targetRunId)}.zip`;
}

async function resolveAnalysisArtifactAbsolutePath(input: {
...
function buildReadme(input: {
  ...
  return `# Analysis run export

This export bundles the stored analysis run state for **${input.studyTitle ?? "Analysis run"}** (${input.runId}).

## Included

- \`manifest.json\`: export manifest and high-level metadata
- \`run/run.json\`: analysis run row snapshot
...
- \`run/estimates.json\`: estimate rows
- \`run/refutations.json\`: stored refutation rows
- \`run/answer-package.json\`: stored analysis answer package when available
- \`run/answers.json\`: grounded analysis answer history
- \`run/compute-runs.json\`: compute telemetry history
- \`run/artifacts.json\`: artifact metadata
- \`artifacts/*\`: downloadable stored artifact files attached to the analysis run

## Notes

- This package mirrors the stored analysis run state; it does not re-run estimation.
- Higher-rung conclusions still depend on the DAG, identification assumptions, and refutation results captured here.
...
function buildComparisonReadme(input: {
  ...
  return `# Analysis run comparison export

This export bundles a comparison snapshot for **${input.studyTitle ?? "Analysis study"}**.
...
- This package compares already stored analysis run state only.
...
async function getAnalysisRunExportSnapshot(input: {
  ...
  if (!run) {
    throw new AnalysisExportError("Analysis run not found.", "analysis_run_not_found");
  }
  ...
  const [[study], [question], [identification], estimands, estimates, refutations, [answerPackage], answers, artifacts, compute] = await Promise.all([
    db.select().from(analysisStudies).where(eq(analysisStudies.id, run.studyId)),
    ...
    db.select().from(analysisEstimands).where(eq(analysisEstimands.runId, run.id)).orderBy(desc(analysisEstimands.createdAt)),
    db.select().from(analysisEstimates).where(eq(analysisEstimates.runId, run.id)).orderBy(desc(analysisEstimates.createdAt)),
    db.select().from(analysisRefutations).where(eq(analysisRefutations.runId, run.id)).orderBy(desc(analysisRefutations.createdAt)),
    db.select().from(analysisAnswerPackages).where(eq(analysisAnswerPackages.runId, run.id)),
...
async function buildArtifactEntries(input: {
  ...
  return Promise.all(
    input.artifacts.map(async (artifact, index) => {
      const absolutePath = await resolveAnalysisArtifactAbsolutePath({
...
export async function exportAnalysisRunZip(input: {
  organizationId: string;
  runId: string;
}) {
  const db = await getAppDatabase();
  const [[organization], snapshot] = await Promise.all([
    ...
    getAnalysisRunExportSnapshot(input),
  ]);

  if (!organization) {
    throw new AnalysisExportError("Organization not found for analysis run export.", "analysis_export_org_missing");
  }
...
export async function exportAnalysisRunComparisonZip(input: {
  baseRunId: string;
  organizationId: string;
  studyId: string;
  targetRunId: string;
  ...
  if (!organization) {
    throw new AnalysisExportError(
      "Organization not found for analysis comparison export.",
      "analysis_export_org_missing",
    );
  }

  if (baseSnapshot.run.studyId !== input.studyId || targetSnapshot.run.studyId !== input.studyId) {
    throw new AnalysisExportError(
      "Both runs must belong to the requested analysis study.",
      "analysis_comparison_study_mismatch",
    );
  }
  ...
  const manifest = {
    ...
    comparison_type: "analysis_run_pair",
    exported_at: Date.now(),
    organization: {
      id: organization.id,
      slug: organization.slug,
