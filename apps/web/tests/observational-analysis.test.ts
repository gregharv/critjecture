...
import { executeObservationalRun, getObservationalRunDetail } from "@/lib/observational-analysis";
...
describe("observational analysis telemetry", () => {
  afterEach(async () => {
    vi.clearAllMocks();
  ...
  it("records compute-run telemetry and links artifacts to observational runs", async () => {
    ...
    try {
      ...
      expect(artifactRows.some((artifact) => artifact.fileName === "observational_result.json")).toBe(true);
      expect(artifactRows.every((artifact) => artifact.storagePath.includes("/observational_runs/"))).toBe(true);

      const detail = await getObservationalRunDetail({
        organizationId: user!.organizationId,
        runId: result.id,
      });
      const packageRows = await db.select().from(predictiveAnswerPackages).where(eq(predictiveAnswerPackages.runId, result.id));

      expect(detail.answerPackage).not.toBeNull();
      expect(packageRows[0]?.id).toBe(`observational-answer-package:${result.id}`);
      ...
      expect(detail.artifacts[0]?.downloadPath).toContain(`/api/analysis/observational/runs/${result.id}/artifacts/`);
    } finally {
  ...
  it("marks observational compute telemetry as failed when execution crashes", async () => {
    mocks.execFileAsync.mockRejectedValueOnce(
      Object.assign(new Error("Observational worker crashed."), {
        stderr: "traceback",
    ...
    try {
      ...
      ).rejects.toThrow("Observational worker crashed.");
      ...
      expect(predictiveRunRows[0]?.summaryText).toBe("Observational worker crashed.");
      expect(computeRows).toHaveLength(1);
      expect(computeRows[0]?.status).toBe("failed");
      expect(computeRows[0]?.failureReason).toBe("Observational worker crashed.");
