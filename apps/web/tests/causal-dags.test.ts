...
describe("causal DAGs", () => {
  ...
  it("stores approval on the exact DAG version and promotes the study to ready_to_run", async () => {
    ...
    try {
      ...
      const approval = await approveCausalDagVersion({
        approvalText: "I confirm this DAG is ready for a causal run.",
        approvedByUserId: user!.id,
        dagId: dag.id,
        dagVersionId: version.dagVersionId,
        organizationId: user!.organizationId,
      ...
      const workspace = await getCausalDagWorkspaceDetail({
        organizationId: user!.organizationId,
        studyId: intake.studyId,
      });
