...
import { createAndExecuteAnalysisRun, getAnalysisRunDetail, listAnalysisRunsForStudy } from "@/lib/analysis-runs";
import {
...
describe("analysis runs", () => {
  ...
  it("creates an identified analysis run with estimands, estimates, refutations, compute runs, and an answer package", async () => {
    ...
    try {
      ...
      const result = await createAndExecuteAnalysisRun({
        runUser: {
          id: user!.id,
          organizationId: user!.organizationId,
      ...
      expect(packageRows[0]?.packageJson).toContain("CORROBORATED HIGHER-RUNG CONJECTURE");
  ...
  it("requires approval for the exact DAG version before creating a run", async () => {
    ...
    try {
      ...
      await createAnalysisDagVersion({
        createdByUserId: user!.id,
        dagId: dag.id,
        draft: {
          assumptions: [],
  ...
  it("generates grounded answers only from stored answer packages and records answer history", async () => {
    ...
    try {
      ...
      expect(answer?.answerText).toContain("stored analysis answer package only");
  ...
  it("returns run summaries with comparison-ready metrics", async () => {
    ...
    try {
      ...
      const summaries = await listAnalysisRunsForStudy({
        organizationId: user!.organizationId,
        studyId,
      });
